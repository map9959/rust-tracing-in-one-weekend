...
impl Camera{
    pub fn new(aspect_ratio: f64, image_width: i32, focal_length: f64, viewport_height: f64, camera_center: Vec3) -> Camera{
        ...
        Camera{image_width, image_height, camera_center, pixel_delta_u, pixel_delta_v, pixel00_loc}
    }
    ...
    pub fn render(&self, scene: &Scene){
        ...
        for j in 0..self.image_height{
            for i in 0..self.image_width{
                let pixel_loc = self.pixel00_loc + (self.pixel_delta_u*i as f64 + self.pixel_delta_v*j as f64);
                let pixel_direction = pixel_loc - self.camera_center;
                
                let camera_ray = Ray::new(self.camera_center, pixel_direction);
                
                let color = Camera::ray_color(camera_ray, scene);
                write_color(color);
            }
            progress_bar.inc(1);
        }
